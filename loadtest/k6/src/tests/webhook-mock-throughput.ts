...
const PRE_ALLOCATED_VUS = parseInt(__ENV.PRE_ALLOCATED_VUS || '20');
const MAX_VUS = parseInt(__ENV.MAX_VUS || '1000');
const WEBHOOK_URL = __ENV.WEBHOOK_URL || 'http://localhost:8080/webhook';
...
export default function () {
  ...
  const params = {
    headers: {
      ...
      'x-outpost-event-id': `test-${__VU}-${__ITER}`,
    },
  };
