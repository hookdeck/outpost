...
describe('Webhook Destinations - Contract Tests (SDK-based validation)', () => {
  ...
  describe('POST /api/v1/tenants/{tenant_id}/destinations - Create Webhook Destination', () => {
    it('should create a webhook destination with valid config', async () => {
      ...
      expect(destination.type).to.equal('webhook');
    ...
    it('should create destination with user-provided ID', async () => {
      const customId = `custom-webhook-${Date.now()}`;
      const destinationData = createWebhookDestination({
        id: customId,
      });
      const destination = await client.createDestination(destinationData);
    ...
    it('should reject creation with missing required config field: url', async () => {
  ...
  describe('GET /api/v1/tenants/{tenant_id}/destinations - List Webhook Destinations', () => {
    ...
    it('should filter destinations by type', async () => {
      const destinations = await client.listDestinations({ type: 'webhook' });

      destinations.forEach((dest) => {
        expect(dest.type).to.equal('webhook');
      });
    });
  });
  ...
  describe('PATCH /api/v1/tenants/{tenant_id}/destinations/{id} - Update Webhook Destination', () => {
    ...
    it('should update destination config', async () => {
      ...
      if (updated.config) {
        expect(updated.config.url).to.equal('https://updated.example.com/webhook');
      }
    });
