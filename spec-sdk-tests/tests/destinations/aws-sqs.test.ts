...
import { createAwsSqsDestination } from '../../factories/destination.factory';
...
describe('AWS SQS Destinations - Contract Tests (SDK-based validation)', () => {
  ...
  describe('POST /api/v1/tenants/{tenant_id}/destinations - Create AWS SQS Destination', () => {
    it('should create an AWS SQS destination with valid config', async () => {
      ...
      expect(destination.type).to.equal('aws_sqs');
    ...
    it('should create destination with user-provided ID', async () => {
      const customId = `custom-sqs-${Date.now()}`;
      const destinationData = createAwsSqsDestination({
        id: customId,
      });
      const destination = await client.createDestination(destinationData);
    ...
    it('should reject creation with missing required config field: queueUrl', async () => {
  ...
  describe('GET /api/v1/tenants/{tenant_id}/destinations - List AWS SQS Destinations', () => {
    before(async () => {
      ...
      await client.createDestination(
        createAwsSqsDestination({
          topics: [TEST_TOPICS[0]],
          config: {
            queueUrl: 'https://sqs.us-west-2.amazonaws.com/123456789012/my-queue-2',
          },
        })
      );
    });
    ...
    it('should filter destinations by type', async () => {
      const destinations = await client.listDestinations({ type: 'aws_sqs' });

      destinations.forEach((dest) => {
        expect(dest.type).to.equal('aws_sqs');
      });
    });
  });
  ...
  describe('PATCH /api/v1/tenants/{tenant_id}/destinations/{id} - Update AWS SQS Destination', () => {
    ...
    it('should update destination topics', async () => {
      ...
      expect(updated.type).to.equal('aws_sqs');
