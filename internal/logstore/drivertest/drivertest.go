...
func RunConformanceTests(t *testing.T, newHarness HarnessMaker) {
	...
	t.Run("TestGenericPaginationSuite", func(t *testing.T) {
		testGenericPaginationSuite(t, newHarness)
	})
...
// testGenericPaginationSuite uses the generic paginationtest.Suite to test
// pagination behavior for both ListEvent and ListDeliveryEvent.
func testGenericPaginationSuite(t *testing.T, newHarness HarnessMaker) {
	t.Helper()
	...
	t.Run("ListDeliveryEvent", func(t *testing.T) {
		// Use mutable state for tenant isolation - Cleanup regenerates IDs before each test
		...
		suite := paginationtest.Suite[*models.DeliveryEvent]{
			...
			Cleanup: func(ctx context.Context) error {
				// Generate fresh tenant/destination for each test to ensure isolation
				tenantID = idgen.String()
				destinationID = idgen.Destination()
				idPrefix = idgen.String()[:8] // Unique prefix for IDs to avoid conflicts
				return nil
			},
	...
	t.Run("ListEvent", func(t *testing.T) {
		...
		suite := paginationtest.Suite[*models.Event]{
			...
			InsertMany: func(ctx context.Context, items []*models.Event) error {
				// ListEvent is populated via InsertManyDeliveryEvent
