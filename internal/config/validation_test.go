...
func TestDestinationsWebhookSignatureTemplates(t *testing.T) {
	tests := []struct {
		name    string
		config  *config.Config
		wantErr string
		...
		},
		{
			...
			config: func() *config.Config {
				c := validConfig()
				c.Destinations.Webhook.SignatureContentTemplate = `{{.Timestamp.Unix}}.{{.Body | join \",\"}}`
				return c
			}(),
		...
		},
		{
			...
			config: func() *config.Config {
				c := validConfig()
				c.Destinations.Webhook.SignatureHeaderTemplate = "t={{.Timestamp},v0={{.Signatures}"
				return c
			}(),
			wantErr: "invalid signature header template",
		},
		{
			name: "content template references a header-only field",
			config: func() *config.Config {
				c := validConfig()
				c.Destinations.Webhook.SignatureContentTemplate = `v1:{{.Timestamp.Unix}}:{{.Signatures | join ","}}`
				return c
			}(),
			wantErr: "can't evaluate field Signatures",
		},
		{
			name: "header template references a content-only field",
			config: func() *config.Config {
				c := validConfig()
				c.Destinations.Webhook.SignatureHeaderTemplate = "v0={{.Body}}"
				return c
			}(),
			wantErr: "can't evaluate field Body",
		},
		{
			name: "standard mode ignores the configured templates",
			config: func() *config.Config {
				c := validConfig()
