...
type IDGenConfig struct {
	...
	DeliveryPrefix      string `yaml:"delivery_prefix" env:"ID_GEN_DELIVERY_PREFIX" desc:"Prefix for delivery IDs, prepended with underscore (e.g., 'dlv_123'). Default: empty (no prefix)" required:"N"`
	DeliveryEventPrefix string `yaml:"delivery_event_prefix" env:"ID_GEN_DELIVERY_EVENT_PREFIX" desc:"Prefix for delivery event IDs, prepended with underscore (e.g., 'dev_123'). Default: empty (no prefix)" required:"N"`
}
