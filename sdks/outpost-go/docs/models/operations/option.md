...
```go
outpostgo.WithServerURL("https://api.example.com")
```
...
```go
outpostgo.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
outpostgo.WithClient(httpClient)
```
...
```go
outpostgo.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
outpostgo.WithTimeout(30 * time.Second)
```
