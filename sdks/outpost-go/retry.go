...
// Retry - Triggers a retry for delivering an event to a destination. The event must exist and the destination must be enabled and match the event's topic.
type Retry struct {
	rootSDK          *Outpost
...
func newRetry(rootSDK *Outpost, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *Retry {
	return &Retry{
		rootSDK:          rootSDK,
...
func (s *Retry) Retry(ctx context.Context, request components.RetryRequest, opts ...operations.Option) (*operations.RetryEventResponse, error) {
