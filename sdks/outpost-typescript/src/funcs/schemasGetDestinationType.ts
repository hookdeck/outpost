 ...
 */
export function schemasGetDestinationType(
  client: OutpostCore,
  type: operations.GetDestinationTypeSchemaType,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.DestinationTypeSchema,
...
> {
  const input: operations.GetDestinationTypeSchemaRequest = {
    type: type,
  };
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.GetDestinationTypeSchemaRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.DestinationTypeSchema$inboundSchema),
