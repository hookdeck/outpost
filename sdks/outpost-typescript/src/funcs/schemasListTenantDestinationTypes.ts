 ...
 */
export function schemasListTenantDestinationTypes(
  client: OutpostCore,
  request: operations.ListTenantDestinationTypeSchemasRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    Array<components.DestinationTypeSchema>,
    | OutpostError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ListTenantDestinationTypeSchemasRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/tenants/{tenant_id}/destination-types")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, z.array(components.DestinationTypeSchema$inboundSchema)),
    M.fail([404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
