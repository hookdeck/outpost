...
Operation events are delivered on a best-effort, at-least-once basis. Each emission retries up to 3 times with exponential backoff (100ms, 200ms, 400ms). Events may be lost if all retries fail — they are not persisted or replayed. Consumers should be prepared to handle duplicate events and use the `id` field for deduplication.
