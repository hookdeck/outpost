...
Operation events are delivered with at-least-once semantics. Each emission retries up to 3 times with exponential backoff (100ms, 200ms, 400ms). Consumers should be prepared to handle duplicate events — use the `id` field for deduplication.
