...
You can customize the follow webhook features and behaviors. The full path to the YAML configuration is prefixed with `outpost.config.destinations.webhook`.
...
| `signature_content_template`       | `DESTINATIONS_WEBHOOK_SIGNATURE_CONTENT_TEMPLATE`       | `t={{.Timestamp.Unix}},v0={{.Signatures \| join ","}}` |
...
3. Ensure any in-flight events are delivered before the migration begins.
