...
SDK v0.13.1 simplifies security scheme names, removes the global `tenant_id` parameter, and flattens method signatures.

### Security scheme names

The SDK security field names have been simplified:
...
client := outpostgo.New(
    outpostgo.WithSecurity(components.Security{
        APIKey: outpostgo.String(apiKey),
    }),
)
```
...
// After — tenantId passed per method
const outpost = new Outpost({
  security: { jwt },
  serverURL: `${SERVER_URL}/api/v1`,
});
const destinations = await outpost.destinations.list({ tenantId: "my-tenant" });
```
...
1. Rename `adminApiKey` to `apiKey` and `tenantJwt` to `jwt` in SDK security configuration.
2. Remove `tenantId` from the SDK constructor and pass it to each method call instead.
3. Rename `getDestinationTypeJwt` to `getDestinationType` and `listDestinationTypesJwt` to `listDestinationTypes`.
